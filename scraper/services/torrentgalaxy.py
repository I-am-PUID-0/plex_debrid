        ...
        try:
            ...
            if status_code == 200:
                ...
                if torrentList:
                    ...
                    for count, torrent in enumerate(torrentList):
                        ...
                        title = re.sub(
                            r"[^\w\s\.\-]", "", title
                        )  # Remove non-alphanumeric characters except for dots, spaces, and hyphens
                        title = title.replace(" ", ".")
                        ...
                        if re.match(r"(" + altquery.replace(".", "\.").replace("\.*", ".*") + ")", title, re.I):
                            ...
                            seeders_leechers_element = torrent.select_one(
                                'div.tgxtablecell span[title="Seeders/Leechers"]'
                            )
                            seeders = (
                                int(
                                    seeders_leechers_element.getText()
                                    .strip()
                                    .replace(",", "")
                                    .replace("[", "")
                                    .split("/")[0]
                                )
                                if seeders_leechers_element
                                else 0
                            )
                            if regex.search(r"([0-9]*?\.[0-9]+)(?= MB)", size, regex.I):
                                size = (
                                    regex.search(r"([0-9]*,?[0-9]*?\.[0-9]+)(?= MB)", size, regex.I)
                                    .group()
                                    .replace(",", "")
                                )
                            ...
                            scraped_releases += [
                                releases.release(
                                    "[torrentgalaxy]", "torrent", title, [], size, [download], seeders=seeders
                                )
                            ]
                            ui_print(
                                f"[torrentgalaxy] Scraped release: title={title}, size={size:.2f} GB, seeders={seeders}",
                                ui_settings.debug,
                            )
                else:
                ...
                ui_print(
                    "[torrentgalaxy] Failed to retrieve the page. Status code: " + str(status_code), ui_settings.debug
                )
            ...
            if hasattr(response, "status_code") and not str(response.status_code).startswith("2"):
                ui_print(
                    ...
                    + ": torrentgalaxy is temporarily not reachable"
                )
            else:
