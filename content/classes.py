...
class ignore:
    ...
    def add(self):
        for service in ignore():
            ui_print(
                "ignoring item of type '" + self.__module__ + "' on service '" + service.__module__ + "'",
                ui_settings.debug,
            )
    ...
    def remove(self):
        for service in ignore():
            ui_print(
                "un-ignoring item of type '" + self.__module__ + "' on service '" + service.__module__ + "'",
                ui_settings.debug,
            )
            self.match(service.__module__)
            service.remove(self)
...
class media:
    ...
    def match(self, service):
        ...
        if not service in self.services:
            ...
            if not service == "content.services.textfile":
                ui_print(
                    "matching item: '" + query + "' of service '" + self.__module__ + "' to service '" + service + "'",
                    ui_settings.debug,
                )
    ...
    def query(self, title=""):
        ...
        if self.type == "movie":
            if regex.search(
                str(self.year), releases.rename(self.title.replace(str(self.year), "") + " " + str(self.year))
            ):
                                ...
                                if n == 4
                                    ...
                                    if n == 5
                                        ...
                                        if n == 6
                                            ...
                                            "VII"
                                            if n == 7
                                            else "VIII" if n == 8 else "IX" if n == 9 else "X" if n == 10 else str(n)
                                        )
                                    )
                                )
                            )
            ...
            if self.type == "show":
                ...
                if hasattr(self, "Seasons"):
                    for season in self.Seasons:
                        ...
                        season.anime_count = (
                            "[0-9][^0-9]?-[^0-9]?0*(" + str(self.anime_count) + "|" + str(self.anime_count + 1) + ")"
                        )
                ...
                if not self.query() + " [" + version.name + " upgrade]" in media.downloaded_versions:
                    for rule in version.rules:
                        if not rule[1] == "upgrade":
                            continue
                        if releases.sort.version.rule(rule[0], rule[1], rule[2], rule[3]).upgrade(
                            self.existing_releases
                        ):
                                    ...
                                    for episode in season.Episodes:
                                        if self == episode:
                                            try:
                                                ...
                                                ui_print(
                                                    "error: (file name exception): " + self.query() + " " + str(e),
                                                    ui_settings.debug,
                                                )
                                                return
                ...
                if self.type == "show":
                    ...
                    for release in releases:
                        if (
                            ...
                            elif datetime.datetime.strptime(release_date, "%Y-%m-%d") > datetime.datetime.strptime(
                    ...
                    if release_date == None:
                        for release in releases:
                            if release_date == None:
                                ...
                                release_date = release.release_date
                        release_date = datetime.datetime.strptime(release_date, "%Y-%m-%d") + datetime.timedelta(
                            days=60
                        )
                    ...
                    if hasattr(self, "offset_airtime"):
                        for offset in self.offset_airtime:
                            ...
                            ui_print(
                                ...
                                + (" (including offset of: " + offset + "h)" if offset != "0" else "")
                            )
                        return False
                    ...
                    if hasattr(self, "offset_airtime"):
                        for offset in self.offset_airtime:
                            if datetime.datetime.utcnow() > datetime.datetime.strptime(
                                ...
                                return True
                            available = (
                                datetime.datetime.strptime(self.first_aired, "%Y-%m-%dT%H:%M:%S.000Z")
                                + datetime.timedelta(hours=float(offset))
                                - datetime.datetime.utcnow()
                            )
                            ui_print(
    ...
    def collect(self, refresh_=True):
        for refresh_service in refresh():
            if refresh_service.__module__ == self.__module__ or (
                self.__module__
                in ["content.services.trakt", "releases", "content.services.overseerr", "content.services.plex"]
                and refresh_service.__module__ in ["content.services.plex", "content.services.jellyfin"]
            ):
                if refresh_ or refresh_service.name == "Plex Labels":
                    refresh_service(self)
            elif (
                self.__module__ in ["content.services.plex", "content.services.overseerr"]
                        ...
                        while len(self.Releases) == 0 and i <= retries:
                            for k, title in enumerate(self.alternate_titles):
                                self.Releases += scraper.scrape(
                                    self.query(title).replace(str(self.year), str(year)),
                                    self.deviation(year=str(year)) + "(" + imdbID + ")?",
                                )
                ...
                if len(self.Seasons) > 0:
                    ...
                    if len(self.Seasons) > 1:
                        if self.isanime():
                            ...
                            for k, title in enumerate(self.alternate_titles[:3]):
                                self.Releases += scraper.scrape(
                                    self.query(title), self.deviation() + "(" + imdbID + ")?"
                                )
                                if len(self.Releases) < 20 and k == 0 and not imdb_scraped and not imdbID == ".":
                                        ...
                                        if match:
                                            for index, season_query in enumerate(season_queries):
                                                if regex.match(season_query, release.title, regex.I):
                                                    if (
                                                        version.wanted >= len(self.Seasons[index].files()) - 2
                                                        and season_releases[index] == None
                                                    ):
            ...
            if len(self.Episodes) > 2:
                ...
                if not debrid_downloaded:
                    ...
                    if len(self.Releases) == 0:
                        for k, title in enumerate(self.alternate_titles[:3]):
                            self.Releases += scraper.scrape(
                                self.query(title)[:-1], "(.*|S" + str("{:02d}".format(self.index)) + "|" + imdbID + ")"
                            )
                            if len(self.Releases) < 20 and k == 0 and not imdb_scraped and not imdbID == ".":
                                self.Releases += scraper.scrape(
            ...
            if (not debrid_downloaded or retry) and not hasattr(self, "skip_scraping"):
                ...
                if self.isanime():
                    for title in self.alternate_titles[:3]:
                        self.Releases += scraper.scrape(
                            self.anime_query(title),
                            self.deviation() + "(" + imdbID + ")?(nyaa" + "|".join(self.alternate_titles) + ")?",
                        )
                        if len(self.Releases) > 0:
                            break
                if len(self.Releases) == 0 or not self.isanime():
                    for title in self.alternate_titles[:3]:
                        if self.isanime():
                            self.Releases += scraper.scrape(
                                self.query(title).replace(".", " "), self.deviation() + "(" + imdbID + ")?"
                            )
                        else:
            ...
            for version in self.versions():
                ...
                for release in copy.deepcopy(self.Releases):
                    ...
                    if (hasattr(release, "cached") and len(release.cached) > 0) or (
                        hasattr(release, "maybe_cached") and len(release.maybe_cached) > 0
                    ):
    ...
    def bitrate(self):
        ...
        try:
            ...
            ui_print(
                ...
                + self.type
                + " duration: "
                + "{:02d}h:{:02d}m".format(int(duration / 1000) // 3600, (int(duration / 1000) % 3600) // 60),
                ui_settings.debug,
            )
        except:
