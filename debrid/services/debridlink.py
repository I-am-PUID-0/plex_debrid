...
def logerror(response):
    if not response.status_code == 200:
        ui_print(
            "[debridlink] error " + str(response.status_code) + ": " + str(response.content), debug=ui_settings.debug
        )
...
def oauth(code=""):
    if code == "":
        response = post(
            ...
            f"client_id={client_id}&scope=get.post.seedbox%20get.post.delete.seedbox",
        )
            ...
            if not debrid_uncached:  # Cached Download Method for Debrid-Link
                ...
                try:
                    ...
                    if hasattr(response, "success") and response.success:
                        if (
                            hasattr(response, "value")
                            and hasattr(response.value, "downloadPercent")
                            and response.value.downloadPercent == 100
                        ):
                    ...
                    if torrent_id is not None:
                        ...
                        if hasattr(response, "success") and not response.success:
                            ...
                            if hasattr(response, "error") and response.error == "access_denied":
                                ui_print(
                                    f"[debridlink] insufficient permissions to delete uncached torrent; try to reset your API key."
                                )
            else:
...
def check(element, force=False):
    ...
    for release in element.Releases[:]:
        ...
        release.maybe_cached += ["DL"]


def add_torrent(link: str, isasync: bool = False):
    return post(
        "https://debrid-link.fr/api/v2/seedbox/add", f"url={requests.utils.quote(link)}&async={str(isasync).lower()}"
    )
