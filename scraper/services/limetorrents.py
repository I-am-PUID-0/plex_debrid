            ...
            if status_code == 200:
                ...
                if torrentList:
                    ...
                    for count, torrent in enumerate(torrentList):
                        ...
                        title = regex.sub(
                            r"[^\w\s\.\-]", "", title
                        )  # a good place for this is in the classes.py during the regex bits that checks for matches
                        title = title.replace(" ", ".")
                        ...
                        if regex.match(r"(" + altquery.replace(".", r"\.").replace(r"\.*", ".*") + ")", title, regex.I):
                            ...
                            download = link.replace("http://itorrents.org/torrent/", "magnet:?xt=urn:btih:").replace(
                                ".torrent?title=", "&dn="
                            )
                            ...
                            scraped_releases += [
                                releases.release(
                                    "[limetorrents]", "torrent", title, [], size, [download], seeders=seeders
                                )
                            ]
                            ui_print(
                                f"[limetorrents] Scraped release: title={title}, size={size:.2f} GB, seeders={seeders}",
                                ui_settings.debug,
                            )
                else:
                ...
                ui_print(
                    "[limetorrents] Failed to retrieve the page. Status code: " + str(status_code), ui_settings.debug
                )

        except Exception as e:
            if hasattr(response, "status_code") and not str(response.status_code).startswith("2"):
                ui_print(
                    "[limetorrents] error " + str(response.status_code) + ": limetorrents is temporarily not reachable"
                )
            else:
