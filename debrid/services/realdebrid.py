...
# (required) Authentication of the Debrid service, can be oauth as well. Create a setting for the required variables in the ui.settings_list. For an oauth example check the trakt authentication.
api_key = ""
# Define Variables
    ...
    if not response.status_code in [200, 201, 204]:
        ...
        for error in errors:
            if response.status_code == error[0]:
                desc = error[1]
        ui_print(
            "[realdebrid] error: (" + str(response.status_code) + desc + ") " + str(response.content),
    ...
    for release in cached[:]:
        try:  # if release matches query
            if regex.match(query, release.title, regex.I) or force:
                response = post(
                    "https://api.real-debrid.com/rest/1.0/torrents/addMagnet", {"magnet": release.download[0]}
                )
                if hasattr(response, "error") and response.error == "infringing_file":
                    ui_print(
                        f"[realdebrid]: torrent {release.title} marked as infringing... looking for another release."
                    )
                    continue
                ...
                if hasattr(response, "files") and len(response.files) > 0:
                    ...
                    for file_ in response.files:
                        debrid_file = file(
                            file_.id, file_.path, file_.bytes, release.wanted_patterns, release.unwanted_patterns
                        )
                    ...
                    cached_ids = [
                        ...
                        if vf.wanted and not vf.unwanted and vf.name.endswith(tuple(media_file_extensions))
                    ]
                    if len(cached_ids) == 0:
                        ...
                        if response.status in ["queued", "magnet_conversion", "downloading", "uploading"]:
                            if hasattr(element, "version"):
                                ...
                                if debrid_uncached:
                                    ...
                                    ui_print(
                                        f"[realdebrid]: {release.title} is in {response.status} status (not cached). Looking for another release."
                                    )
                                    delete("https://api.real-debrid.com/rest/1.0/torrents/delete/" + torrent_id)
                                    continue
                        else:
                            ui_print(
                                f"[realdebrid]: {release.title} is in status [{response.status}] - trying a different release."
                            )
                            delete("https://api.real-debrid.com/rest/1.0/torrents/delete/" + torrent_id)
                            continue
                    ...
                    if response.status == "downloading":
                        ...
                        ui_print(
                            f"[realdebrid]: no files found for torrent {release.title} in status {response.status}. looking for another release."
                        )
                        delete("https://api.real-debrid.com/rest/1.0/torrents/delete/" + torrent_id)
                        continue
                ...
                ui_print(
                    f'[realdebrid] error: rejecting release: "{release.title}" because it doesnt match the allowed deviation "{query}"'
                )
...
def check(element, force=False):
    ...
    for release in element.Releases[:]:
        ...
        # we won't know if it's cached until we attempt to download it
        release.maybe_cached += ["RD"]
