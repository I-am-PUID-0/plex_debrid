...
def logo(path='', update=""):
    ...
    print(
        '/_/               /_____/                         [v' + ui_settings.version[0] + ']' + update)
    print()
            ...
            try:
                with open(config_dir + '/plex_debrid.log', 'a') as f:
                    if string == 'done' and sameline_log:
                        ...
                        f.write('done' + '\n')
                        f.write(
                            '[' + str(datetime.datetime.now().strftime("%d/%m/%y %H:%M:%S")) + '] ' + string + ' ')
                        sameline_log = True
                ...
                print('[' + str(datetime.datetime.now().strftime("%d/%m/%y %H:%M:%S")) +
                      '] logging error: couldnt write into log file at: ' + config_dir + '/plex_debrid.log')
        # ui
        if debug == "true":
            if string == 'done' and sameline:
                ...
                print('done')
                print(
                    '[' + str(datetime.datetime.now().strftime("%d/%m/%y %H:%M:%S")) + '] ' + string, end=' ')
                sameline = True
            elif string.endswith('...'):
                print(
                    '[' + str(datetime.datetime.now().strftime("%d/%m/%y %H:%M:%S")) + '] ' + string, end=' ')
