                ...
                if torrents and (torrents[0].get("name") == "No results returned" or "total_found" in torrents[0]):
                    ...
                    for torrent in torrents:
                        ...
                        if regex.match(r"(" + altquery.replace(".", "\.").replace("\.*", ".*") + ")", title, regex.I):
                            scraped_releases += [
                                releases.release(
                                    "[thepiratebay]", "torrent", title, [], size, [download], seeders=seeders
                                )
                            ]
                            ui_print(
                                f"[thepiratebay] Scraped release: title={title}, size={size:.2f} GB, seeders={seeders}",
                                ui_settings.debug,
                            )
            else:
                ui_print(
                    "[thepiratebay] Failed to retrieve data from API. Status code: " + str(status_code),
                    ui_settings.debug,
                )
