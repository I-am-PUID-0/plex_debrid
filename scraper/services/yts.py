...
def scrape(query, altquery):
    mediatype = (
        "TV" if regex.search(r"(\bseries|S\d{2}|complete|season|seasons|nyaa|part\b)", altquery, regex.I) else "Movies"
    )
    ...
    if imdb_id is None:
        ...
        imdb_id = match.group(1) if match else None
    ui_print(
        "[yts] using IMDB ID: " + "tt" + imdb_id if imdb_id else "[yts] No IMDB ID found in query", ui_settings.debug
    )
            ...
            if response.status_code == 200:
                ...
                for movie in movies:
                    ...
                    for torrent in torrents:
                        ...
                        release_title = (
                            f"{title}.{year}.{quality}.{torrent_type}.{video_codec}.{bit_depth}bit.AAC.{audio_channels}"
                        )

                        scraped_releases += [
                            releases.release(
                                "[yts]", "torrent", release_title, [], size, [download], seeders=int(seeders)
                            )
                        ]

                        ui_print(
                            f"[yts] Scraped release: title={release_title}, size={size:.2f} GB, seeders={seeders}",
                            ui_settings.debug,
                        )
            else:
