...
def scrape(query, altquery):
    ...
    try:
        ...
        if not response.status_code == 200:
            ui_print(
                "[zilean] error " + str(response.status_code) + ": failed response from zilean. " + response.content
            )
            return []
        ...
        ui_print(
            "[zilean] error: zilean couldn't be reached. Make sure your zilean base url ["
            + base_url
