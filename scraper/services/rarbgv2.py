...
def scrape(query, altquery):
    ...
    if "rarbgv2" in active:
        ...
        url = (
            "http://therarbg.com/get-posts/keywords:"
            + urllib.parse.quote(query.replace(".", " ").replace("?", ""), safe=":/")
            + "/"
        )
        response = None
        try:
                ...
                if torrentList:
                    ...
                    for count, torrent in enumerate(torrentList):
                        ...
                        if regex.match(r"(" + altquery.replace(".", r"\.").replace(r"\.*", ".*") + ")", title, regex.I):
                            ...
                            scraped_releases += [
                                releases.release(
                                    "[rarbg]", "torrent", title, [], size, [download], seeders=int(seeders)
                                )
                            ]
                            ui_print(
                                f"[rarbg] Scraped release: title={title}, size={size} GB, seeders={seeders}",
                                ui_settings.debug,
                            )
                else:
