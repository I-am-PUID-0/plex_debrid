...
def setup(self):
    ...
    while not working_url:
        ...
        print(
            "Please provide your overseerr base URL in the following format 'http://overseerr:5055' or press enter to return to the main menu."
        )
        print()
            ...
            try:
                ...
                for element in response.results:
                    if (
                        ...
                        and (element.requestedBy.displayName in users or users == ["all"])
                        and (
        ...
        if len(users) > 0 and len(api_key) > 0:
            ...
            try:
                ...
                for element_ in response.results:
                    if (
                        not any(x.id == element_.id and x.updatedAt == element_.updatedAt for x in last_requests)
                        and (element_.requestedBy.displayName in users or users == ["all"])
                        and (
                        ...
                        try:
                            ...
                            ui_print(
                                "[overseerr] error: couldnt match item to service " + matching_service,
                                ui_settings.debug,
                            )
                        element.request_id = element_.media.id
                        if not element in self.data:
