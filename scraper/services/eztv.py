...
def scrape(query, altquery):
    ...
    if "eztv" in active:
        ui_print(
            "[eztv] using extended query: "
            + query.replace(".?", "").replace("'", "").replace("’", "").strip(".").strip(" "),
            ui_settings.debug,
        )
        headers = {
        ...
        try:
            ...
            ui_print(
                "[eztv] Received response for search query with status code: " + str(status_code), ui_settings.debug
            )
                ...
                if torrentList:
                    ...
                    for count, torrent in enumerate(torrentList):
                        ...
                        if regex.match(r"(" + altquery.replace(".", "\.").replace("\.*", ".*") + ")", title, regex.I):
                            ...
                            scraped_releases += [
                                releases.release("[eztv]", "torrent", title, [], size, [download], seeders=int(seeders))
                            ]
                            ui_print(
                                f"[eztv] Scraped release: title={title}, size={size} GB, seeders={seeders}",
                                ui_settings.debug,
                            )
                else:
