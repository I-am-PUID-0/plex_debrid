...
def request(func, *args):
    try:
        ...
        if hasattr(response, "status_code") and response.status_code != 200:
            ui_print(
                f'[comet] error {str(response.status_code)}: failed response from comet. {response.content.decode("utf-8")}'
            )
            return []
...
def scrape_imdb_movie(session: requests.Session, base_url: str, base64_config: str, imdb_id: str) -> list:
    return collate_releases_from_response(
        request(get, session, f"{base_url}/{base64_config}/stream/movie/{imdb_id}.json")
    )


def scrape_imdb_series(
    session: requests.Session, base_url: str, base64_config: str, imdb_id: str, season: int = 1, episode: int = 1
) -> list:
    return collate_releases_from_response(
        request(get, session, f"{base_url}/{base64_config}/stream/series/{imdb_id}:{str(season)}:{str(episode)}.json")
    )
...
def collate_releases_from_response(response: requests.Response) -> list:
    ...
    for result in response.streams:
        ...
        if hasattr(result, "description") and (
            result.description == "Invalid Comet config."
            or regex.search(r"(?<=Invalid )(.*)(?= account)", result.description)
        ):
        ...
        try:
            ...
            scraped_releases += [releases.release("[comet: " + source + "]", "torrent", title, [], size, links, seeds)]
        except Exception as e:
