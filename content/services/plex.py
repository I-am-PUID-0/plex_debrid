        ...
        if hasattr(item, "user"):
            if isinstance(item.user[0], list):
                ...
                try:
                    ...
                    ui_print(
                        '[plex] error: item "' + item.title + '" couldnt be removed from ' + user[0] + "`s watchlist"
                    )
                if not self == []:
                    self.data.remove(item)
...
class library(classes.library):
                    ...
                    if choice0 == "1":
                        ...
                        if choice == "add":
                            ...
                            if choice in indices:
                                ...
                                print()
                                print(
                                    "Successfully added plex library section '" + sections[int(choice) - 1].title + "'"
                                )
                                print()
                                time.sleep(3)
            else:
        ...
        def call(element):
            try:
                ...
                if library_item == None:
                    ui_print(
                        '[plex] error: couldnt add labels - item: "'
                        + element.query()
    ...
    def __new__(self, silent=False):
        ...
        for section, types in sections:
            ...
            for type in types:
                url = (
                    library.url + "/library/sections/" + section + "/all?type=" + type + "&X-Plex-Token=" + users[0][1]
                )
                response = get(url)
                if hasattr(response, "MediaContainer"):
                    if hasattr(response.MediaContainer, "Metadata"):
            ...
            if len(section_response) == 0:
                ui_print(
                    f"[plex error]: local plex library section [{section}]: {section_title} at server address: {library.url} is empty!"
                )
                continue
            else:
        ...
        if len(list_) - len(current_library) > 0:
            ui_print("done")
            ui_print(
                "[plex] getting metadata for " + str(len(list_) - len(current_library)) + " collected movies/shows ..."
            )
        updated = False
        for item in list_:
            try:
