    ...
    if code == "":
        ...
        while response == None:
            response = post2(
                ...
                json.dumps({"code": code, "client_id": client_id, "client_secret": client_secret}),
            )
            time.sleep(1)
...
class watchlist(classes.watchlist):
    ...
    def remove(self, original_element):
        ...
        try:
            response = post(
                "https://api.trakt.tv/sync/watchlist/remove", json.dumps(data, default=lambda o: o.__dict__)
            )
            ...
            if len(p_lists) > 0:
                ...
                for list in p_lists:
                    ...
                    if not p_list_id == None:
                        response = post(
                            "https://api.trakt.tv/users/me/lists/" + str(p_list_id) + "/items/remove",
...
class show(classes.media):
    def __init__(self, other):
        ...
        try:
            ...
            self.originallyAvailableAt = datetime.datetime.utcnow().strftime("%Y-%m-%d")
        response, header = get("https://api.trakt.tv/shows/" + str(self.ids.trakt) + "/seasons?extended=episodes,full")
        leafCount = 0
    ...
    class ignore(classes.ignore):
        ...
        def remove(self):
            ...
            try:
                ...
                data = {"movies": movies, "shows": shows, "seasons": seasons, "episodes": episodes}
                response = post(
                    "https://api.trakt.tv/sync/history/remove", json.dumps(data, default=lambda o: o.__dict__)
                )
