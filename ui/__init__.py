...
def ignored():
    ...
    while not back:
        ...
        if choice in indices:
            print(
                "Media item: " + content.classes.ignore.ignored[int(choice) - 1].query() + " removed from ignored list."
            )
...
def load(doprint=False, updated=False):
    ...
    if "version" not in settings:
        ...
        update(settings, ui_settings.version)
        updated = True
    # compatibility code for updating from <2.10
    ...
    # compatibility code for updating from <2.20
...
def update_available():
    try:
        response = requests.get(
            "https://raw.githubusercontent.com/elfhosted/plex_debrid/main/ui/ui_settings.py", timeout=0.25
        )
                    ...
                    if t1 - t0 >= 5:
                        if plex_watchlist.update() or overseerr_requests.update() or trakt_watchlist.update():
                            ...
                            try:
                                ...
                                ui_print(
                                    "couldnt sort monitored media by newest, using default order.", ui_settings.debug
                                )
                            new_watchlists = unique(new_watchlists)
                            for element in new_watchlists[:]:
                                if element in watchlists:
