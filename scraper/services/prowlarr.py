...
def scrape(query, altquery):
    ...
    if "prowlarr" in active:
        ...
        try:
            ...
            ui_print(
                "[prowlarr] error: prowlarr couldnt be reached. Make sure your prowlarr base url is correctly formatted (default: http://prowlarr:9696)."
            )
            return []
        if response.status_code == 200:
            ...
            for result in response[:]:
                ...
                if (
                    regex.match(r"(" + altquery.replace(".", "\.").replace("\.*", ".*") + ")", result.title, regex.I)
...
def resolve(result):
    ...
    try:
        ...
        ui_print(
            "[prowlarr] error: resolver couldnt get magnet/torrent for release: " + result.title, ui_settings.debug
        )
        return scraped_releases
